---
title: Java 从零开始（31）Java 集合
zhihu-title-image: https://pica.zhimg.com/v2-e1cf667c04b0f63c15003183ddd03e79_1440w.jpg?source=172ae18b				
zhihu-url: https://zhuanlan.zhihu.com/p/412974858
---
# Java 集合
...
`java.util.Collection`接口的实现可用于存储 Java 对象。例如，的所有学生可以视为一个`Collection`。
...
`java.util.Map`接口的实现可用于表示“键”（key）和“值”（value）对象之间的映射。一个映射表示一组“键”对象，其中每一个“键”对象都映射到一个“值”对象。因此可以通过键来查找值。例如，的每一个学生都有他自己的账户积分，这个关联关系可以用`Map`来表示。
...
* **同时存储具有一对一关系的数据**：例如存储学生的积分，为了方便检索对应学生的积分，可使用`Map`将学生的`uid`和对应的积分进行一对一关联；
...
修改后：arrayList 内容为 ImoocStudent{nickname='小李', position='UI 设计师'}
索引位置 0 的学生的昵称为：小李
索引位置 0 的学生的职位为：UI 设计师
```
...
public class HashSetDemo1 {
    public static void main(String[] args) {
        ...
        hashSet.add("Python");
        // 打印 hashSet 的内容
        System.out.println("hashSet 中的内容为：" + hashSet);
    }
}
```
...
```java
hashSet 中的内容为：[TypeScript, Java, PHP, Python]
```
...
public class HashSetDemo2 {
    public static void main(String[] args) {
        ...
        // 打印 hashSet 的内容
        System.out.println("删除 Python 元素后，hashSet 中的内容为：" + hashSet);
    }
}
```
...
```java
[Java, Python]
删除 Python 元素后，hashSet 中的内容为：[Java]
```
...
public class HashSetDemo4 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("删除 nickname 为 Colorful 的对象后，集合元素为：");
        System.out.println(hashSet);
    }
}
```
...
删除 nickname 为 Colorful 的对象后，集合元素为：
...
public class HashMapDemo1 {
    ...
    public static void main(String[] args) {
        ...
        // 删除 key 为 Java 的数据
        map.remove("Chinese");
        System.out.println("删除键为 Chinese 的映射后，map 内容为：");
        // 打印 map
        System.out.println(map);
        // 修改元素：
        map.put("Java", "一种编程语言");
        System.out.println("修改键为 Java 的值后，Java=" + map.get("Java"));
        // 遍历 map
        System.out.println("通过遍历 entrySet 方法得到 key-value 映射：");
...
删除键为 Chinese 的映射后，map 内容为：
{English=英语，Java=咖啡}
修改键为 Java 的值后，Java=一种编程语言
通过遍历 entrySet 方法得到 key-value 映射：
English - 英语
Java - 一种编程语言
English 键对应的值为：英语
```
