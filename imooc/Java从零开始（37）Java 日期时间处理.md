...
![](//img.mukewang.com/wiki/5ee07fcd09808dc011910735.jpg)
...
新的日期时间 API 吸取了[Joda-Time](https://www.joda.org/joda-time/)的精华，提供了更优秀易用的 API，位于`java.time`包中，包含了`LocalTime`（本地日期）、`LocalDate`（本地时间）、`LocalDateTime`（本地日期时间）、`ZonedDateTime`（带时区的日期时间）和`Duration`（时间间隔）类。
