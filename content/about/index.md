...
- 针对现有系统存在的问题，我们引入了CQRS架构并通过订阅binlog方式将MySQL数据同步到Elasticsearch，最终将搜索服务从MySQL迁移到Elasticsearch，大幅度提升了搜索接口的响应时间，从5秒降低到200毫秒。
- 我们还对数据库进行了优化，确保SQL语句能够命中索引，同时调整了表索引数量，使得查询时间从3秒降低到100毫秒。
- 我们遵循DDD设计思想，利用gRPC和Kratos逐渐将单体架构拆分成微服务架构，提高了系统的弹性和可扩展性。
- 我们优化了缓存策略，增加缓存过期时间，降低了内存占用率，同时通过消费binlog的方式优化数据库和缓存一致性问题，使得系统运行更加稳定并且能够处理更大的负载。
